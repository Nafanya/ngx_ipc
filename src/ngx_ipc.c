...
int ngx_lua_ipc_send_alert(ngx_int_t target_worker, ngx_str_t *name, ngx_str_t *data) {
  int            i;
...
//      DBG("parsing conn_id from '%V', len=%ui", data, data->len);
      conn_id = ngx_atoi(data->data, data->len);
      if (conn_id == NGX_ERROR) {
          DBG("bad conn_id"); //TODO: emerg
      }

//      DBG("sending stats from %ui back to %ui", (ngx_uint_t) ngx_getpid(), sender_pid);
...
//          DBG("couldn't allocate ")
      ...
      if (sender_pid != (ngx_int_t)ngx_getpid()) {
          ngx_lua_ipc_send_alert(sender_pid, &name1, &response);
      }
      ngx_destroy_pool(pool);
